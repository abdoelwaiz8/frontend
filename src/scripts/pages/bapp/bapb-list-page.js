...
export default class BappListPage {
    ...
    return `
      <div class="flex items-center justify-center min-h-screen">
        <div class="text-center">
          ...
          <p class="text-slate-900 font-black uppercase tracking-tight">MEMUAT DAFTAR BAPP...</p>
        </div>
      </div>
    `;
  }
  ...
  async afterRender() {
    try {
      ...
      console.error('Error loading BAPP list:', error);
      this._showError('Gagal memuat daftar BAPP: ' + error.message);
    }
  }
  ...
  async _renderList() {
    ...
    container.innerHTML = `
      <div class="flex flex-col md:flex-row md:justify-between md:items-start mb-8 gap-6">
          <div>
              <h2 class="heading-architectural text-4xl text-slate-900 mb-3">DAFTAR BAPP</h2>
                ...
                <div class="flex flex-col md:flex-row md:items-center gap-6">
                    <div class="flex items-start gap-4 flex-1">
                        <div class="w-14 h-14 bg-${status.bg} border-2 border-slate-900 flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform">
                            <i class="ph-fill ph-briefcase text-white text-2xl"></i>
                        </div>
  ...
  _updatePageTitle() {
    ...
    if (title) title.innerHTML = 'DAFTAR BAPP';
  }
}